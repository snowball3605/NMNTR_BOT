    ...
    public void onSlashCommand(SlashCommandInteractionEvent event) {
        try {
            ...
            List<Message> messageList = event.getChannel().getHistory().retrievePast(Integer.parseInt(String.valueOf(Math.max(1, Math.min(100, 50)))))
                            .complete();
            guild.getTextChannelById("960520009795395584").deleteMessages(messageList).queue();
                EmbedBuilder a = new EmbedBuilder()
                        .setTitle("問題回報|活動參加|疑問解答")
                        .setDescription("你好,如有需要請點擊以下按鈕\n將會有專人解答您的疑問")
                        .setAuthor("NRTS製作團隊")
                        .setColor(new Color(0xFF0000));
                event.getGuild().getTextChannelById("960520009795395584").sendMessageEmbeds(a.build()).setActionRow(Button.success("00000", "\uD83D\uDCDC開啟客服單")).queue();
