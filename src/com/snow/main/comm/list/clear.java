...
import static com.snow.main.util.CommandOption.COUNT;
...
public class clear {
    ...
    public void onSlashCommand(SlashCommandInteractionEvent event) {
        try {
            ...
            event.getInteraction().deferReply(true).setContent("已刪除訊息數量" + message).queue();
            String channel = event.getTextChannel().getName();
            EmbedBuilder a = new EmbedBuilder()
                    .setTitle("刪除訊息")
                    .setDescription("已刪除訊息數量" + message + "\n" + channel)
                    .setTimestamp(Instant.now());
            event.getGuild().getTextChannelById("953659969788575765").sendMessageEmbeds(a.build()).queue();
